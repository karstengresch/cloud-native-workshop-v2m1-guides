...
Let's jump to code containing identified migration issues. Expand the *monolith* source project in the MTA explorer:

image::mta_project_issues.png[mta, 700]
...
image::mta-issues-table.png[mta, 500]

=== 2. Fix the issue with `ApplicationLifecycleListener`

View details about the migration issue. Right-click on `WebLogic ApplicationLifecycleListenerEvent[rule-id:xxx]` in _Hints_ of _StartupListener.java_ file. Click on `View Details`:
